/* 
장바구니 상품추가 C : addCart(), saveCart()
상품불러오기 R : loadCart()
옵션병경 U : 수량변경 updateQty
장바구니 비우기 D : 상품취소 removeItem(),clearCart()
상품 합계계산 : cartTotals()
*/
...
//통화 포멧
const fmatMoney= n=> Number(n).toLocaleString('ko-KR');
...
/**************************
    makeItemKey(id, option) CART
    동일상품, 동일옵션인지 판별.
    옵션이 있는 경우에는 키-값 정렬후에 '|'로연결


**************************/
function makeItemKey({ id,option }){
    //가져온 키, 값을 소문자로 만들고 빈공간은 없앰
    //1.Object.keys(options)을 이용 key 배열을 얻는다. 예)["color","size"]
    //2.키를 정렬(sort())3. 키:값 문자열로 변환 4.|를 이용해 하나로 결합
    const optPart = options ?
        Object.keys(option).sort().map( k=> `${k}:${options[k]}`).join('|')
        :``;
        return `${id}__${option}`;
}
